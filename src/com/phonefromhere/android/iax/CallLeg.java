...
import com.phonefromhere.android.iax.frames.DtmfFrame;
import com.phonefromhere.android.iax.frames.Frame;
import com.phonefromhere.android.iax.frames.FrameType;
import com.phonefromhere.android.iax.frames.FullFrame;
import com.phonefromhere.android.iax.frames.MiniFrame;
import com.phonefromhere.android.iax.frames.TextFrame;
import com.phonefromhere.android.iax.frames.VoiceFrame;
import com.phonefromhere.android.iax.frames.control.ControlFrame;
import com.phonefromhere.android.iax.frames.control.ControlSubclass;
import com.phonefromhere.android.iax.frames.iax.HungupListener;
import com.phonefromhere.android.iax.frames.iax.IaxAckFrame;
import com.phonefromhere.android.iax.frames.iax.IaxFrame;
import com.phonefromhere.android.iax.frames.iax.IaxHangupFrame;
import com.phonefromhere.android.iax.frames.iax.IaxNewFrame;
...
public class CallLeg implements HungupListener, AudioReceiver {
            ...
            while (iter.hasNext() && _isFinished == false) {
                ...
                if (frame.isAcknowlegded()) {
                    ...
                    if (frame.getRetries() > MAX_RETRIES) {
                         ...
                         * call leg.
                         */
                        IaxLog.getLog().iax(this.getClass().getSimpleName() + ".sendFrames(): exceeds MAX_RETRIES "
                                + frame.toString());
                        handleTimeout(false);
                    } else {
    ...
    public void receivedFrame(Frame frame) {
        if (_isFinished) {
            ...
            if (frame.isFullFrame()) {
                IaxLog.getLog().iax(this.getClass().getSimpleName() + ".receivedFrame(): "
                        + frame.toString());
    ...
    protected void receivedDtmfFrame(DtmfFrame dtmfFrame) {
        IaxLog.getLog().iax(this.getClass().getSimpleName() + ".receivedDtmfFrame(): ");
    ...
    protected void receivedTextfFrame(TextFrame textFrame) {
        IaxLog.getLog().iax(this.getClass().getSimpleName() + ".receivedTextfFrame(): ");
            ...
            switch (subclass) {
                case ACCEPT:
                    ...
                    if (_state == CallLegState.WAITING) {
                        MediaFormat format = iaxFrame.getFormat();
                        try {
    ...
    protected void sendHangupFrame(CauseCodeType type, String message) {
        IaxLog.getLog().iax(this.getClass().getSimpleName() + ".sendHangupFrame()");
    ...
    protected void logInvalidStateFrameReceived(Frame frame) {
        IaxLog.getLog().iax("Invalid state " + _state.toString() + " for received frame="
                + frame.toString());
    }

    protected void logInvalidStateFrameSend(String frame) {
        IaxLog.getLog().iax("Invalid state " + _state.toString() + " for sending frame="
                + frame);
    }
}
