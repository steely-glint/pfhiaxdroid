...
/**
 * This class implements some of the basic arithmetic for encoding and decoding
 * gsm.<br/>
...
public class GSM_Base implements GSM_Constant {
    ...
    private final static String version_id = "@(#)$Id: GSM_Base.java,v 1.1 2011/02/03 14:33:14 uid1003 Exp $ Copyright Westhawk Ltd";

    public final GSM_Arithmetic AR = new GSM_Arithmetic();
    ...
    protected void getExponentMantissa(SegmentParam param) {
         ...
         * _xMc[0..12]. This computation uses the fact that the decoded version
         * of _xmaxc can be calculated by using the exponent and the mantissa
         * part of _xmaxc (logarithmic table).
         * 
         * So, this method avoids any division and uses only a scaling of the
         * RPE samples by a function of the exponent. A direct multiplication by
         * the inverse of the mantissa (NRFAC[0..7] found in table 5.5) gives
         * the 3 bit coded version _xMc[0..12] of the RPE samples.
         */

        // Compute exponent and mantissa of the decoded version of _xmaxc.
        param._exp = 0;
        if (param._xmaxc > 15) {
            var1 = GSM_Arithmetic.toShort(param._xmaxc >> 3);
            param._exp = GSM_Arithmetic.s_sub(var1, (short) 1);
        }
        var2 = GSM_Arithmetic.toShort(param._exp << 3);
        param._mant = GSM_Arithmetic.s_sub(param._xmaxc, var2);
        ...
        if (param._mant == 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if (itest == 0) {
                    var1 = GSM_Arithmetic.toShort(param._mant << 1);
                    param._mant = GSM_Arithmetic.s_add(var1, (short) 1);
                    /*
                     * } if (itest == 0) {
                     */
                    param._exp = GSM_Arithmetic.s_sub(param._exp, (short) 1);
                }
            }
        }
        param._mant = GSM_Arithmetic.s_sub(param._mant, (short) 8);
    }

    /**
     * Procedure 5.2.16 APCM inverse quantization This part is for decoding the
     * RPE sequence of coded _xMc[0..12] samples to obtain the xMp[0..12] array.
     * Table 5.6 is used to get the mantissa of _xmaxc (FAC[0..7]).
     */
    // 4x per frame
    protected short[] apcmInverseQuantization(SegmentParam param) {
        ...
        // See 5.2.15 for _exp
        temp2 = GSM_Arithmetic.s_sub((short) 6, param._exp);
        temp3 = GSM_Arithmetic.toShort(1 << GSM_Arithmetic.s_sub(temp2,
                (short) 1));
        for (int i = 0; i < 13; i++) {
            // This subtraction is used to restore the sign of _xMc[i].
            var1 = GSM_Arithmetic.toShort(param._xMc[i] << 1);
            temp = GSM_Arithmetic.s_sub(var1, (short) 7);
            temp = GSM_Arithmetic.toShort(temp << 12);
            temp = GSM_Arithmetic.s_mult_r(temp1, temp);
            temp = GSM_Arithmetic.s_add(temp, temp3);
            xMp[i] = GSM_Arithmetic.toShort(temp >> temp2);
        }
        return xMp;
    }
     ...
     * ep[0..39] for the LTP analysis filter. The inputs are the _Mc which is
     * the grid position selection and the xMp[0..12] decoded RPE samples which
     * are upsampled by a factor of 3 by inserting zero values.
     */
    // 4x per frame
    protected short[] rpeGridPositioning(short xMp[], SegmentParam param) {
        short ep[] = new short[SEGMENT];
        /*
         * // No need, new short[] inits all to zero for (int k=0; k<SEGMENT;
         * k++) { ep[k] = 0; }
         */
        for (int i = 0; i < 13; i++) {
    ...
    protected short[] decodeLAR(short LARc[], short LARpp[]) {
        ...
        for (int i = 1; i < 9; i++) {
            ...
            // _LAR[i].
            temp1 = GSM_Arithmetic.toShort(GSM_Arithmetic
                    .s_add(LARc[i], MIC[i]) << 10);
            temp2 = GSM_Arithmetic.toShort(B[i] << 1);
            temp1 = GSM_Arithmetic.s_sub(temp1, temp2);
            temp1 = GSM_Arithmetic.s_mult_r(INVA[i], temp1);
            LARpp[i] = GSM_Arithmetic.s_add(temp1, temp1);
        }
     ...
     * set of decoded LARs (LARpp(j)). <br/>
     * <br/>
     * Procedure 5.2.9.1 Interpolation of the LARpp[1..8] to get the LARp[1..8]<br/>
     * <br/>
     */
     ...
     */
    protected short[] interpolationLARpp_1(short LARpp_j_1[], short LARpp_j[],
            short LARp[]) {
        short var1, var2;
        for (int i = 1; i < 9; i++) {
            var1 = GSM_Arithmetic.toShort(LARpp_j_1[i] >> 2);
            var2 = GSM_Arithmetic.toShort(LARpp_j[i] >> 2);
            LARp[i] = GSM_Arithmetic.s_add(var1, var2);

            var2 = GSM_Arithmetic.toShort(LARpp_j_1[i] >> 1);
            LARp[i] = GSM_Arithmetic.s_add(LARp[i], var2);
        }
        return LARp;
    }
     ...
     */
    protected short[] interpolationLARpp_2(short LARpp_j_1[], short LARpp_j[],
            short LARp[]) {
        short var1, var2;
        for (int i = 1; i < 9; i++) {
            var1 = GSM_Arithmetic.toShort(LARpp_j_1[i] >> 1);
            var2 = GSM_Arithmetic.toShort(LARpp_j[i] >> 1);
            LARp[i] = GSM_Arithmetic.s_add(var1, var2);
        }
        return LARp;
    }
     ...
     */
    protected short[] interpolationLARpp_3(short LARpp_j_1[], short LARpp_j[],
            short LARp[]) {
        short var1, var2;
        for (int i = 1; i < 9; i++) {
            var1 = GSM_Arithmetic.toShort(LARpp_j_1[i] >> 2);
     ...
     */
    protected short[] interpolationLARpp_4(short LARpp_j_1[], short LARpp_j[],
            short LARp[]) {
        // TODO:
        // use memcopy?
        for (int i = 1; i < 9; i++) {
    ...
    protected short[] computeRP(short LARp[], short rp[]) {
        short temp, var1;
        for (int i = 1; i < 9; i++) {
            temp = GSM_Arithmetic.s_abs(LARp[i]);
            if (temp < 11059) {
                temp = GSM_Arithmetic.toShort(temp << 1);
            } else if (temp < 20070) {
                temp = GSM_Arithmetic.s_add(temp, (short) 11059);
            } else {
                var1 = GSM_Arithmetic.toShort(temp >> 2);
                temp = GSM_Arithmetic.s_add(var1, (short) 26112);
            }
            rp[i] = temp;
            if (LARp[i] < 0) {
                rp[i] = GSM_Arithmetic.s_sub((short) 0, rp[i]);
            }
        }
        return rp;
    }
    ...
    public short readShort(DataInputStream in) throws java.io.IOException {
        short s;
        if (LITTLE_ENDIAN) {
     ...
     * @param bitno
     *            The position of the bit (in output)
     */
     ...
     * @param in_noLSB
     *            The number of LSB in input to copy
     * @param output
     *            The output array to copy the bits to
     * @param out_pos
     *            The start position in output
     ...
     * @param bitno
     *            The position of the bit (in input)
     * @return one or zero
     */
    ...
    public void writeCodedParams(DataOutputStream out)
            throws java.io.IOException {
        /*
         * LSB Variable Frame 6 _LAR[1] 1 6 _LAR[2] 2 5 _LAR[3] 3 5 _LAR[4] 4 4
         * _LAR[5] 5 4 _LAR[6] 6 3 _LAR[7] 7 3 _LAR[8] 8
         */

        /*
         * 4x: LSB Variable Seg1 Seg2 7 _Nc 9 26 2 _bc 10 27 2 _Mc 11 28 6
         * _xmaxc 12 29 3 _xMc[0..12] 13-25 30-42
         */
        // NOT GSM_HEADER!!!
    ...
    public void readCodedParams(DataInputStream in) throws java.io.IOException {
        /*
         * LSB Variable Frame 6 _LAR[1] 1 6 _LAR[2] 2 5 _LAR[3] 3 5 _LAR[4] 4 4
         * _LAR[5] 5 4 _LAR[6] 6 3 _LAR[7] 7 3 _LAR[8] 8
         */
