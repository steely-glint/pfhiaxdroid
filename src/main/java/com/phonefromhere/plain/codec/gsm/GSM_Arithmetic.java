...
/**
 * This class implements the basic arithmetic for gsm. Chapter 5.1 of:
 * 
 ...
 * 
 * @see <a
 *      href="http://java.sun.com/docs/books/tutorial/java/nutsandbolts/datatypes.html">java.sun.com
 *      tutorial</a>
 * 
...
/*
 * Short on 16 bits: -1 * 2^15 (= -32768) < X < (2^15) - 1 (= +32767)
 * 
 * Integer on 32 bits: -1 * 2^31 (= -2147483648) < X < (2^31) - 1 (=
 * +2147483647)
 */
public class GSM_Arithmetic {
    @SuppressWarnings("unused")
    private final static String version_id = "@(#)$Id: GSM_Arithmetic.java,v 1.1 2011/02/03 14:33:14 uid1003 Exp $ Copyright Westhawk Ltd";

    /**
     * Performs the addition (s_var1+s_var2) with overflow control and
     * saturation; the result is set at +32767 (=2^15-1) when overflow occurs or
     * at -32768 (=-2^15) when underflow occurs.
     */
    ...
    /**
     * 32 bits addition of two 32 bits variables (i_var1 + i_var2) with overflow
     * control and saturation; the result is set at 2147483647 (=2^31-1) when
     * overflow occurs and at -2147483648 (=-2^31) when underflow occurs.
     */
    ...
    /**
     * 32 bits subtraction of two 32 bits variables (i_var1 - i_var2) with
     * overflow control and saturation; the result is set at 2147483647
     * (=2^31-1) when overflow occurs and at -2147483648 (=-2^31) when underflow
     * occurs.
     */
     ...
     * and s_mult(-32768, -32768) = 32767 (=2^15-1) i.e. s_mult(-2^15, -2^15) =
     * 2^15-1
     */
    ...
    /**
     * i_mult is a 32 bit result for the multiplication of s_var1 times s_var2
     * with a one bit shift left.<br/>
     * i_mult(s_var1, s_var2) = (s_var1 times s_var2) &lt;&lt; 1.<br/>
     * The condition i_mult(-32768, -32768) does not occur in the algorithm.
     */
     ...
     * (16384 = 2^14) i.e. s_mult_r(-2^15, -2^15) = 2^15-1
     */
    ...
    /**
     * s_div produces a result which is the fractional integer division of
     * s_var1 by s_var2; s_var1 and s_var2 shall be positive and s_var2 shall be
     * greater or equal to s_var1; The result is positive (leading bit equal to
     * 0) and truncated to 16 bits. <br/>
     * If (s_var1 == s_var2) then s_div(s_var1, s_var2) = 32767 (=2^15-1) <br/>
    ...
    /**
     * norm produces the number of left shifts needed to normalize the 32 bits
     * variable i_var1 for positive values on the interval with minimum of
     * 1073741824 (=2^30) and maximum of 2147483647 (=2^31-1) and for negative
     * values on the interval with minimum of -2147483648 (=-2^31) and maximum
     * of -1073741824 (=-2^30); in order to normalize the result, the following
     * operation shall be done:<br/>
    ...
    /**
     * i_var2 = s_var1:<br/>
     * Deposit the 16 bits of s_var1 in the LSB 16 bits of i_var2 with sign
     * extension.
     */
    ...
    /**
     * s_var1 = i_var1:<br/>
     * With overflow control and saturation; the result is set at +32767
     * (=2^15-1) when overflow occurs or at -32768 (=-2^15) when underflow
     * occurs.
     */
    ...
    /**
     * i_var1 = l_var1:<br/>
     * With overflow control and saturation; the result is set at 2147483647
     * (=2^31-1) when overflow occurs and at -2147483648 (=-2^31) when underflow
     * occurs.
     */
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\nTesting i_add():");
        i_result = i_add(Integer.MAX_VALUE, 1);
        ...
        i_result = i_add(Integer.MIN_VALUE, -1);
        ...
        System.out.println("\nTesting i_sub():");
        i_result = i_sub(Integer.MIN_VALUE, 1);
        System.out.println("Integer.MIN_VALUE - 1 = " + i_result);
        ...
        i_result = i_sub(Integer.MAX_VALUE, 1);
        System.out.println("Integer.MAX_VALUE - 1 = " + i_result);
        ...
        System.out.println("\nTesting norm():");
        int vars[] = { Integer.MIN_VALUE, Integer.MIN_VALUE / 2,
                Integer.MAX_VALUE, Integer.MAX_VALUE / 2, 0, 1, -1, 7, -7 };
        ...
        for (int i = 0; i < vars.length; i++) {
            ...
            if (i_var1 > 0) {
                if ((1073741824 <= i_norm_var1) && (i_norm_var1 <= 2147483647)) {
            ...
            System.out
                    .println(i_norm_var1 + " = " + i_var1 + " << " + s_result);
        }
